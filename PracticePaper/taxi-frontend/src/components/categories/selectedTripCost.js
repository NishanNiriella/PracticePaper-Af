...
class SelectedTripCost extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(`http://localhost:8088/vehicle/${this.props.match.params.id}`).then(response => {
            this.setState({selectedVehicle: response.data.data});
        });

        axios.get(`http://localhost:8088/category/v2/${this.props.match.params.cid}`).then(response => {
    ...
    onSubmit(e) {
        ...
        axios.post('http://localhost:8088/category/calculate', data).then(response => {
    ...
    render() {
        return (
            <div className="container">
                ...
                <h3>Vehicle,<br/>&nbsp;&nbsp;{this.state.selectedVehicle.code}<br/>&nbsp;&nbsp;{this.state.selectedVehicle.model} {this.state.selectedVehicle.name}
                </h3>
                <br/>
                ...
                <br/>
                <h2>Trip charge costs: LKR {this.displayCost()}</h2>
            </div>
        )
    }
}
